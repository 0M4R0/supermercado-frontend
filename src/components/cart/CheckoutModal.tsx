...
export const CheckoutModal = ({ onClose, onComplete }: CheckoutModalProps) => {
  ...
  const [selectedPayment, setSelectedPayment] = useState<
    { type: "cash"; id: number } | { type: "card"; card: SavedCard } | null
  >(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const defaultLoc =
          ubicacionesData.find((u) => u.por_defecto) ?? ubicacionesData[0];
        if (defaultLoc) setSelectedUbicacion(defaultLoc);

        const catalogoData = await fetchMetodosPagoCatalogo(token).catch(
          () => null,
        );
  ...
  const cashMethod =
    metodosCatalogo.length > 0
      ? (metodosCatalogo.find(
          (m) =>
            m.nombre.toLowerCase() === "efectivo" ||
            m.nombre.toLowerCase() === "cash" ||
            m.nombre.toLowerCase() === "contado",
        ) ?? metodosCatalogo[0])
      : { id: 1, nombre: "Efectivo" };

  const cardCatalogMethod =
    metodosCatalogo.length > 0
      ? (metodosCatalogo.find(
          (m) =>
            m.nombre.toLowerCase() !== "efectivo" &&
  ...
  const handleCloseConfirm = () => {
    const shouldClose = window.confirm(
      "Estas seguro de que quieres salir y perder tu progreso?",
    );
      ...
      {(["location", "payment", "review"] as Step[]).map((s, i) => {
        ...
        return (
          <div key={s} className="flex items-center gap-2">
            ...
            {i < 2 && (
              <div
                className={`w-8 h-0.5 ${idx < currentIndex ? "bg-green-600" : "bg-gray-200"}`}
              />
            )}
          </div>
        );
            ...
            {paymentLoading ? (
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {cashMethod && (
                  <button
                    onClick={() =>
                      setSelectedPayment({ type: "cash", id: cashMethod.id })
                    }
                    className={`w-full text-left p-3 rounded-xl border-2 transition cursor-pointer ${
                ...
                {savedCards.map((card) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border-2 transition cursor-pointer ${
                      ...
                      selectedPayment.card.id === card.id
                        ? "border-green-500 bg-green-50"
                        : "border-gray-200 hover:border-gray-300"
                    }`}
                  >
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <p className="text-sm text-red-600 mb-3 bg-red-50 border border-red-100 p-3 rounded-xl">
            {error}
          </p>
        )}
